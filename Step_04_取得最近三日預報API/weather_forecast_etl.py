...
def parse_numeric_value(time_item: dict[str, Any]) -> float | None:
    """解析 CWA 時間區間中的第一個可轉數值。"""
    values = time_item.get("ElementValue") or time_item.get("elementValue") or []
    if isinstance(values, dict):
        values = [values]
    if not isinstance(values, list) or not values:
        return None
    item = values[0] if isinstance(values[0], dict) else {}
    merged = dict(time_item)
    merged.update(item)
    for key in (
        "value", "Value", "Temperature", "RelativeHumidity", "WindSpeed",
        "UVIndex", "UV", "ProbabilityOfPrecipitation",
    ):
        raw = merged.get(key)
...
def parse_text_value(time_item: dict[str, Any]) -> str | None:
    """解析 CWA 時間區間中的文字值，例如天氣現象或綜合描述。"""
    values = time_item.get("ElementValue") or time_item.get("elementValue") or []
    if isinstance(values, dict):
        values = [values]
    if not isinstance(values, list) or not values:
    ...
    merged = dict(time_item)
    merged.update(item)
    for key in ("value", "Value", "Weather", "WeatherDescription"):
        raw = merged.get(key)
        if raw is not None and str(raw).strip() != "":
...
def enrich_numeric_from_weather_description(bucket: dict[str, Any], description: str | None) -> None:
    """當 CWA 僅回傳綜合描述時，從描述補出 ML 所需數值欄位。"""
    if not description:
        return
    if bucket.get("temperature_c") is None:
        match = re.search(r"溫度(?:攝氏)?\s*([+-]?\d+(?:\.\d+)?)\s*度", description)
        if match:
            bucket["temperature_c"] = float(match.group(1))
    if bucket.get("relative_humidity_pct") is None:
        match = re.search(r"相對濕度\s*([+-]?\d+(?:\.\d+)?)\s*%", description)
        if match:
            bucket["relative_humidity_pct"] = float(match.group(1))
    if bucket.get("wind_speed_ms") is None:
        match = re.search(r"每秒\s*([+-]?\d+(?:\.\d+)?)\s*公尺", description)
        if match:
            bucket["wind_speed_ms"] = float(match.group(1))


def parse_forecast_rows(payload: dict[str, Any], location_sk: int, county_name: str, town_name: str, run_id: int, batch_issue_time: datetime) -> list[dict[str, Any]]:
    """將多個 CWA 天氣元素依時間區間整合成單筆預報資料。"""
    ...
    buckets: dict[tuple[datetime, datetime], dict[str, Any]] = {}
    elements = get_weather_elements(location_record)
    # 先建立 startTime/endTime 預報桶，再把 DataTime 點值對應進去。
    elements = sorted(
        elements,
        key=lambda element: 1 if any(
            (item.get("DataTime") or item.get("dataTime"))
            and not (item.get("startTime") or item.get("StartTime") or item.get("endTime") or item.get("EndTime"))
            for item in get_time_list(element)
        ) else 0,
    )
    for element in elements:
        name = str(element.get("elementName") or element.get("ElementName") or "").strip()
        lower = name.lower()
        for item in get_time_list(element):
            # CWA F-D0047-061 的溫度／濕度／風速使用 DataTime，
            # 降雨機率與天氣描述使用 startTime/endTime；統一映射到 3 小時桶。
            start_value = (
                item.get("startTime")
                or item.get("StartTime")
                or item.get("DataTime")
            )
            end_value = item.get("endTime") or item.get("EndTime")
            is_point_time = bool((item.get("DataTime") or item.get("dataTime")) and not end_value)
            start = parse_dt(start_value)
            end = parse_dt(end_value) if end_value else None
            if start and not end:
                end = start + timedelta(hours=3)
            if not start or not end:
                continue
            bucket_key = (start, end)
            if is_point_time and buckets and bucket_key not in buckets:
                continue
            bucket = buckets.setdefault(bucket_key, {})
            value = parse_numeric_value(item)
            text_value = parse_text_value(item)
            if "降雨機率" in name or "probability" in lower or "pop" in lower:
                bucket["pop_value"] = value
                bucket["pop_type"] = name
            elif name == "溫度" or lower == "temperature":
                bucket["temperature_c"] = value
            elif "相對濕度" in name or "humidity" in lower:
                bucket["relative_humidity_pct"] = value
            elif "風速" in name or "wind" in lower:
                bucket["wind_speed_ms"] = value
            elif "紫外線" in name or "uv" in lower:
                bucket["uv_index"] = value
            elif "天氣預報綜合描述" in name or "weatherdescription" in lower:
                # 若已有天氣現象（如「多雲」），保留簡短顯示文字；
                # 綜合描述仍用於補出溫度、濕度、風速等特徵。
                if text_value and not bucket.get("weather_description"):
                    bucket["weather_description"] = text_value
                enrich_numeric_from_weather_description(bucket, text_value)
            elif "天氣現象" in name or "weather" in lower:
                if text_value and not bucket.get("weather_description"):
                    bucket["weather_description"] = text_value
    rows = []
    for (start, end), data in sorted(buckets.items()):
        pop = data.get("pop_value")
        if pop is None:
            pop = 0.0
        rows.append({
            "run_id": run_id,
            ...
            "pop_type": data.get("pop_type", "降雨機率"),
            "pop_value": pop,
            "dispatch_recommended": bool(pop >= 30),
            ...
            "rain_risk_flag": bool(pop >= 30),
            "cloudy_risk_flag": bool(pop >= 50),
            "loaded_at": loaded_at,
        })
    non_null = {
        field: sum(1 for row in rows if row.get(field) is not None)
        for field in ("temperature_c", "relative_humidity_pct", "wind_speed_ms", "weather_description", "uv_index")
    }
    print(
        f"[DEBUG] location_sk={location_sk}, county={county_name}, town={town_name}, "
        f"parsed_rows={len(rows)}, non_null={non_null}"
    )
    return rows
# =========================================================
# 寫入前刪除既有資料
