...
PREDICTION_TYPE = os.getenv("PVESPS_PREDICTION_TYPE", "historical_backtest").strip().lower()

# 預設抓最新 N 天做 scoring
SCORING_DAYS = int(os.getenv("PVESPS_SCORING_DAYS", "14"))
...
def read_scoring_input(
    ...
    # 回測只能使用已知 target；day_ahead 嚴禁混入未來實際值。
    if prediction_type == "historical_backtest":
        base_filter += "\n  AND target_available = TRUE"
    elif prediction_type == "day_ahead":
        base_filter += """
  AND target_available = FALSE
  AND training_date::date > (
      SELECT MAX(training_date::date) FROM mart.ml_training_generation_daily
      WHERE target_available = TRUE
  )
        """
    else:
        raise ValueError("PVESPS_PREDICTION_TYPE 必須是 historical_backtest 或 day_ahead")
...
def main() -> None:
    ...
    try:
        ...
        print(f"[INFO] scoring window = {date_from} ~ {date_to}; type={PREDICTION_TYPE}")
        if PREDICTION_TYPE == "day_ahead":
            print("[安全檢查] 僅使用 Cut-off 之後且 target_available=FALSE 的特徵列。")
